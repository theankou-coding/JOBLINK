...
export interface JobDetailProps {
  ...
  timestamp?: number;
  views?: number;
  employerId?: string; // Add employerId to fetch user data
}
...
export default function JobDetail({
  ...
  timestamp,
  views = 0,
  employerId // Get employerId from props
}: JobDetailProps) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          setUserData({
            ...
            profession: user.profession || user.businessName || businessName || "Employer",
            email: user.email,
            phone: user.phone,
            bio: user.bio,
            rating: user.rating,
          ...
          // Fallback to props if user not found
          setUserData({
            username: authorName || profileName || "Employer",
            photoURL: authorPhoto || profileImage || null,
    ...
    fetchUserData();
  }, [employerId, authorName, profileName, authorPhoto, profileImage, businessName]);
  ...
  if (salaryRange && !salary) {
    // Extract numbers from salaryRange (e.g., "$200" -> "200", "$200 - $500" -> "200")
              ...
              {images.length > 0 ? (
                <div className="relative group">
                  <div className="relative aspect-video lg:aspect-2/1 bg-slate-100">
                    <img
                      src={images[currentImage] || "https://via.placeholder.com/1200x600?text=No+Image"}
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200/50 shadow-sm w-full mb-10">
                {loadingUser ? (
                  ...
                  <>
                    <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-900 text-lg truncate">
                          {userData?.username || authorName || profileName || "Employer"}
                        </h4>
                    ...
                    {/* User Stats / Info */}
                    {userData && (userData.email || userData.phone || userData.rating) && (
                      <div className="mb-6 space-y-3">
                        {userData.email && (
                          <div className="text-sm">
                            <span className="text-slate-500">Email: </span>
                            <span className="font-medium text-slate-700 truncate block">{userData.email}</span>
                          </div>
                        )}
                        {userData.rating && (
                          <div className="text-sm">
                            <span className="text-slate-500">Rating: </span>
                            <span className="font-medium text-slate-700">{userData.rating}/5 ⭐</span>
                          </div>
                        )}
                      </div>
                    )}
